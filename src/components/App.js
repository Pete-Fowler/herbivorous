...
function App() {
  ...
  function fetch1() {
    fetch(
      `https://api.spoonacular.com/recipes/complexSearch?apiKey=${key}&sort=random&number=100&diet=vegan&type=main_course`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  function fetch2() {
    fetch(
      `https://api.spoonacular.com/recipes/${recipes[recipeIndex].id}/card?apiKey=${key}`
    )
      .then((res) => res.json())
  ...
  return (
    <div id="app">
      ...
      <Routes>
        ...
        <Route
          path="/saved"
          element={<Saved recipes={savedRecipes} removeRecipe={removeRecipe} />}
        />
