...
function Search({ addRecipe, removeRecipe, results, setResults }) {
  ...
  function handleSubmit(e) {
    ...
    setLoaded("start");
    fetch(
      `https://api.spoonacular.com/recipes/complexSearch?apiKey=${key}&sort=${data.sort}&number=10&diet=vegan&query=${data.string}`
    )
      .then((res) => res.json())
      .then((data) => {
        setResults(data);
  ...
  function getResults(newOffset) {
    setLoaded("start");
    fetch(
      `https://api.spoonacular.com/recipes/complexSearch?apiKey=${key}&offset=${newOffset}&sort=${data.sort}&number=10&diet=vegan&query=${data.string}`
    )
      .then((res) => res.json())
      .then((data) => {
        setResults(data);
    ...
    <div id="search">
      ...
      {loaded === "start" ? (
        ...
        <>
          ...
          />
          <div id="arrows-box">
            <span className="arrow" onClick={back}>
              {"<"}
            </span>
            <span className="arrow" onClick={forward}>
              {">"}
            </span>
          </div>
          {`Page ${offset / 10 + 1} of ${Math.floor(
